...
function create() {
	...
	//var distanceCollisionGroup = game.physics.p2.createCollisionGroup();

	//nextClick = game.time.now;

	//add distance line objects
	//[x1,y1,x2,y2]
	var dLines = [[705.5,10.5,704.5,145.5],
	[785.5,10.5,785.5,146.5],
	[859.5,9.5,860.5,146.5],
	[932.5,8.5,934.5,147.5],
	[1033.5,35.5,966.5,155.5],
	[1115.5, 140.5,  979.5,  176.5],
	[1123.5,215.5,  983.5,  234.5],
	[1123.5, 309.5,  985.5,  304.5],
	[1103.5,  403.5,  976.5,  350.5],
	[952.5,  369.5,  1025.5,  489.5],
	[916.5,  371.5,  935.5,  508.5],
	[820.5,  411.5,  900.5,  521.5],
	[757.5,  499.5,  887.5,  548.5],
	[746.5,  571.5,  877.5,  617.5],
	[735,  607,  820.5,  714.5],
	[697.5,  623.5,  730.5,  760.5],
	[635.5,  622.5,  637.5,  760.5],
	[561.5,  625.5,  557.5,  760.5],
	[485.5,  624.5,  485.5,  759.5],
	[414.5,  626.5,  411.5,  761.5],
	[338.5,  627.5,  339.5,  758.5],
	[263.5,  625.5,  261.5,  760.5],
	[196.5,  624.5,  184.5,  759.5],
	[169.5,  610.5,  67.5,  704.5],
	[25.5,  642.5,  147.5,  579.5],
	[15.5,  533.5,  151.5,  528.5],
	[13.5,  456.5,  151.5,  454.5],
	[14.5,  378.5,  149.5,  379.5],
	[12.5,  308.5,  151.5,  308.5],
	[13.5,  238.5,  150.5,  238.5],
	[25.5,  135.5,  151.5,  185.5],
	[82.5,  52.5,  172.5,  157.5],
	[203.5,  10.5,  202.5,  144.5],
	[279.5,  10.5,  280.5,  146.5],
	[348.5,  9.5,  345.5,  142.5],
	[417.5,  9.5,  421.5,  143.5],
	[494.5,  9.5,  496.5,  145.5],
	[566.5, 8.5,  565.5,  144.5]]
...
//Add the distance lines. Currently the angled ones show as large boxes instead of rotated thing boxes
	for(var i =0; i<dLines.length;i++){
		distanceLines[i] = game.add.sprite((dLines[i][0]+dLines[i][2])/2,(dLines[i][1]+dLines[i][3])/2,null);
		game.physics.p2.enable(distanceLines[i],true);
		distanceLines[i].body.sprite.name ="dLine";
		distanceLines[i].body.static=true;
		var rotation = Math.acos((Math.sqrt(Math.pow(dLines[i][2],2)+Math.pow(dLines[i][3],2)))/(Math.sqrt(Math.pow(dLines[i][0]-dLines[i][2],2)+Math.pow(dLines[i][1]-dLines[i][3],2)))
)*0.01745;
		//distanceLines[i].body.clearShapes();
		distanceLines[i].body.setRectangle(Math.abs(dLines[i][0]-dLines[i][2]),Math.abs(dLines[i][1]-dLines[i][3]),0,0,rotation);
		distanceLines[i].body.data.shapes[0].sensor=true;
		//distanceLines[i].body.setCollisionGroup(distanceCollisionGroup);
		//distanceLines[i].body.collides([carCollisionGroup]);
	}

	car.body.onBeginContact.add(carCollision, this); // check for car hitting wall

	//car.body.collides(distanceCollisionGroup,calcDist,this);
}
...
function render(){
	 ...
	 game.debug.text("best lap: " + bestLap(), 200, 400);
